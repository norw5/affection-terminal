// Verified on PulseChain Blockscout: atropacoin at (unknown)
...
contract atropacoin is ERC20, ERC20Burnable, Ownable {
    ...
    constructor() ERC20(/*name short=*/ unicode"GIMME FIVE", /*symbol long=*/ unicode"⑤") {
        DaiToken = IERC20(dai);
        USDCToken = IERC20(usdc);
        USDTToken = IERC20(usdt);
        transferOwnership(INDEPENDENCEContract);
    }

    function BuyWithDAI() public {
        bool success1 = DaiToken.transferFrom(msg.sender, address(this), 5 * 10 ** decimals());
        require(success1, unicode"Need Approved 5 DAI");
        _mint(msg.sender, 1 * 10 ** decimals());
    }

    function BuyWithUSDC() public {
        bool success1 = USDCToken.transferFrom(msg.sender, address(this), 5 * 10 ** decimals());
        require(success1, unicode"Need Approved 5 USDC");
        _mint(msg.sender, 1 * 10 ** decimals());
    }

    function BuyWithUSDT() public {
        bool success1 = USDTToken.transferFrom(msg.sender, address(this), 5 * 10 ** decimals());
        require(success1, unicode"Need Approved 5 USDT");
        _mint(msg.sender, 1 * 10 ** decimals());
    }
}